...
clamp (full2vc) set-up with linear leak fitting to synthetic data from full2vc
...
savedir = 'out/syn-full2vclinleak-full2vclinleak'
if not os.path.isdir(savedir):
    os.makedirs(savedir)
savedir2 = 'out/syn-full2vclinleak-full2vclinleak-true'
...
# Generate syn. data
#
model_full2vc = m.Model('../mmt-model-files/full2-voltage-clamp-ikr-linleak.mmt',
                protocol_def=protocol_def,
                temperature=273.15 + temperature,  # K
...
trueparams, fixparams = syn.generate_full2vc(model_full2vc, times,
        icell=int(cell), add_linleak=True, return_parameters=True)
...
data = syn.generate_full2vc(model_full2vc, times, icell=int(cell),
...
plt.savefig('%s/data-%s.png' % (savedir, cell), bbox_inches='tight')
plt.close()
...
priorparams = np.append(full2vc_typical_values, 1e-2)
