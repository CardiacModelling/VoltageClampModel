...
LOG="./log/fit-fixkinetics-simvclinleak-loosebound-fixELeak-$FILENAME"
mkdir -p $LOG
...
do
	echo "${CELLS[x]}"
	nohup python fit-fix-kinetics-simvclinleak.py $FILENAME ${CELLS[x]} 10 > $LOG/${CELLS[x]}.log 2>&1 &
