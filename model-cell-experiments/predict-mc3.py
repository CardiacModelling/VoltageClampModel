...
axes[0].plot(times, Vc, ls='-', c='#7f7f7f', label=r'Input $V_{cmd}$')
axes[0].plot(times, Vm, ls='--', c='#bd0026', label=r'Predicted $V_{m}$')
axes[0].set_xlim([times[0], times[-1]])
axes[0].set_ylabel('Voltage (mV)', fontsize=15)
#axes[0].set_xticks([])
axes[0].legend(loc='lower left', bbox_to_anchor=(-0.01, 1.02), ncol=3, fontsize=14, bbox_transform=axes[0].transAxes)

axes[1].plot(times, data, label=r'Measurement ($I_{out}$)')
axes[1].plot(times, Iout, ls='--', label=current_label)
axes[1].set_ylim([-800, 1200])  # TODO?
axes[1].set_xlim([times[0], times[-1]])
axes[1].legend(loc='lower right', bbox_to_anchor=(1.01, 1.02), ncol=2, fontsize=14, bbox_transform=axes[0].transAxes)
axes[1].set_ylabel('Current (pA)', fontsize=15)
